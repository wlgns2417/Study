...
if not core.empty and "expression" in core.columns:
    for _, r in core.iterrows():
        k = str(r.get("expression","")).strip().lower()
        if k: core_map[k] = r
...
def offline_meaning(exp):
    ...
    if k in core_map:
        ...
        if m and m.lower() != "nan": return m
    if k in CURATED: return CURATED[k][0]
    if k in PHRASAL: return PHRASAL[k]
    ...
    if parts: return " ".join(parts) + " · 문맥에 따라 자연스럽게 해석"
...
def example_for(exp):
    ...
    if k in core_map:
        ...
        if ex and ex.lower() != "nan": return ex
    if k in CURATED: return CURATED[k][1]
    if k.endswith(" to"): return f"I {k} go home."
...
if q:
    qq=q.strip().lower()
    f=f[f["expression"].str.lower().str.contains(qq,regex=False) | f["meaning_ko"].str.lower().str.contains(qq,regex=False)]

pages=max(1, math.ceil(len(f)/per))
if "page" not in st.session_state: st.session_state.page=1
st.session_state.page=max(1,min(int(st.session_state.page),pages))
...
st.markdown(f'<div class="page-caption">전체 {len(f):,}개 표현 · 현재 {len(view)}개 표시 · {page}/{pages} 페이지</div>',unsafe_allow_html=True)
...
btns=st.columns(7)
with btns[0]:
    if st.button("◀ 이전",use_container_width=True,disabled=(page<=1)):
        st.session_state.page=page-1
        st.rerun()

for idx,p in enumerate(range(block_start,block_end+1),start=1):
    with btns[idx]:
        if st.button(("● "+str(p)) if p==page else str(p),key=f"page_{p}",use_container_width=True):
            st.session_state.page=p
            st.rerun()

with btns[6]:
    if st.button("다음 ▶",use_container_width=True,disabled=(page>=pages)):
        st.session_state.page=page+1
        st.rerun()
