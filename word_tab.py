...
# 코퍼스 사전의 대표 뜻이 회화 문맥과 다른 경우를 위한 핵심 단어 교정표입니다.
# 생성 CSV의 빈도/점수/순위는 유지하고 화면의 뜻·예문·품사만 자연스럽게 보정합니다.
CURATED_WORDS = {
    ...
    "suppose": ("~라고 생각하다 / ~인 것 같다", "I suppose we could try again.", "verb", "B1"),
    ...
    "eventually": ("결국 / 마침내", "We'll figure it out eventually.", "adverb", "B1"),
    ...
    "consider": ("고려하다 / 생각해 보다", "Have you considered moving closer?", "verb", "A2"),
    "depend": ("달려 있다 / 의존하다", "It depends on what you want.", "verb", "B1"),
    "deserve": ("~할 자격이 있다 / 마땅히 받다", "You deserve a break.", "verb", "B1"),
    ...
    "mention": ("언급하다", "Did she mention my name?", "verb", "A2"),
    "notice": ("알아차리다 / 눈치채다", "Did you notice anything different?", "verb", "A2"),
    "pretend": ("~인 척하다", "Don't pretend you didn't know.", "verb", "B2"),
    "promise": ("약속하다", "I promise I won't tell anyone.", "verb", "A2"),
    "remind": ("상기시키다 / 알려주다", "Remind me to call him later.", "verb", "B1"),
    "seem": ("~처럼 보이다 / ~인 것 같다", "You seem a little tired today.", "verb", "A2"),
    "suggest": ("제안하다", "I suggest we leave early.", "verb", "A2"),
    "wonder": ("궁금하다 / ~일까 생각하다", "I wonder why he left.", "verb", "B1"),
    "admit": ("인정하다", "I have to admit, you were right.", "verb", "A2"),
    "assume": ("추정하다 / 당연하다고 생각하다", "I assumed you already knew.", "verb", "B2"),
    "handle": ("처리하다 / 감당하다", "Don't worry, I can handle it.", "verb", "A2"),
    ...
    "available": ("시간이 되는 / 이용 가능한", "Are you available this afternoon?", "adjective", "B1"),
    ...
    "ridiculous": ("말도 안 되는 / 터무니없는", "That's absolutely ridiculous.", "adjective", "B1"),
...
def _seed_dataframe():
    rows = []
    for rank, (word, values) in enumerate(CURATED_WORDS.items(), 1):
        meaning, example, pos, cefr = values
        rows.append({
            "rank": rank, "word": word, "meaning_ko": meaning, "example_en": example,
            "pos": pos, "cefr": cefr, "score": "preview", "source": "starter preview",
        })
...
def apply_conversation_polish(df: pd.DataFrame) -> pd.DataFrame:
    """빈도 순위는 유지하면서 핵심 회화어의 뜻/예문/품사를 회화 문맥에 맞게 교정합니다."""
    out = df.copy()
    if "word" not in out.columns:
        return out
    keys = out["word"].astype(str).str.lower()
    for word, (meaning, example, pos, cefr) in CURATED_WORDS.items():
        mask = keys.eq(word)
        if mask.any():
            out.loc[mask, "meaning_ko"] = meaning
            out.loc[mask, "example_en"] = example
            if "pos" in out.columns:
                out.loc[mask, "pos"] = pos
            if "cefr" in out.columns:
                out.loc[mask, "cefr"] = cefr
    return out
...
def render_word_tab(base_dir: Path):
    ...
    f = apply_conversation_polish(words)
    ...
    page_input = st.number_input(
        "단어 페이지 직접 이동", min_value=1, max_value=pages,
        value=st.session_state.word_page, step=1, key="word_page_input",
    )
    ...
    for i, (_, r) in enumerate(view.iterrows()):
        ...
        meta_parts = []
        if "pos" in r and pd.notna(r.get("pos")) and str(r.get("pos")).lower() != "other":
        ...
        card = (
            '<div class="card word-card"><div class="e">' + word + '</div>' + meta_html
            + '<div class="ko">' + ko + '</div><div class="ex">💬 ' + ex + '</div></div>'
        )
        with cols[i % 5]:
